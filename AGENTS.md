...
Threadline 是个人任务工作台，提供桌面浏览器、iPhone PWA 和 Windows 桌面应用三种使用方式。项目以同一套 Next.js App Router 前端承载 UI 与业务逻辑；Web/PWA 保持 Next.js 服务模式，Windows 端由 Electron 加载独立静态导出的前端。
...
- `electron/`：Electron Main、Preload、受限 protocol 与 Windows 原生窗口实现。

修改时可以根据任务需要重写整套前端，包括为桌面端建立新的前端实现；重写前后都必须明确架构边界、同步维护 README/docs，并完成与影响范围相称的验证。涉及 Next.js 或 Electron 行为时，应先查阅项目已安装版本的官方文档与当前实现。
