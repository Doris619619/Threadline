...
首次登录后可选择主题、性别和本机自启动，已有账号补做一次。男生隐藏节律，设置中可修改；Windows 正式安装版支持开机自启动。首页改为任务大厅，并移除副标题。详见 [首次使用与个人设置](docs/首次使用与个人设置.md)。

手机新增日程的项目、任务名称与时间输入按行对齐；近期交互改进与更新说明见 [0.1.7 发布说明](docs/release-0.1.7.md)。
