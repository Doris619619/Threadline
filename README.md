...
- 首页 Daily 的父子圆圈、完整名称、预计/实际耗时、今日结果与记录入口全部直接展示，不折叠或移入详情。父级可直接打卡，完成任一子项也算当天完成；取消父级会清除当天全部勾选，保留耗时与结果。子项预计分钟只读，实际逐项输入、父级汇总；无子项时直接填写 Daily 实际，旧父级额外耗时保留并明确计入合计。输入失焦保存，正式记录等待保存成功，失败保留输入并可重试。
