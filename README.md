...
- **字体与外观**：「设置 → 外观」可独立选择默认蓝色／安妮雅主题和默认字体／思源黑体／思源宋体，按设备保存，同源标签页同步；两套主题都跟随系统明暗。手机正文 17px、页面标题 28px，并支持文字放大。字体与角色插画本地分发，没有下载 Apple 字体；真机 Dynamic Type 与捏合缩放仍单独验收。详见[外观说明](docs/appearance.md)。
- **数据层**：Supabase 是任务、项目、Daily、历史、工作站与 Rhythm 的唯一业务真源；未配置时显示明确门禁，不回退本地业务数据。Annotation 笔迹、高亮颜色和窗口 UI 状态仍仅保存在设备上。
- **桌面小窗**：迷你今日只显示当天日程；工作站保存有序任务引用。两者按内容自动变高，超限内部滚动；收起入口可拖到左右屏幕边缘，记住位置并始终置顶，只点击展开。启动及登录阶段提供最小化/关闭按钮并居中显示。
...
- 开始与结束时间支持 `1420` / `14:20` 输入，不支持跨午夜。预计时长独立输入整数分钟，可不填或清空为“待定”；显示为 `45min` / `1h` / `1h30min`，修改起止时间不会覆盖预计。日程、待安排和迷你今日遵循相同规则。
...
- **Windows 桌面三态工作流**：仅 Electron 检测到受限 Main Renderer bridge 时，完整工作台才显示迷你今日、工作站、最小化、原生最大化/还原和关闭。Web/PWA 始终保持完整工作台，历史桌面偏好也不会制造无作用入口。两种紧凑视图使用同一个始终置顶的主窗口并可互转；可拖到左右侧的 edge tab 只是紧凑视图的收起状态，单击恢复最近视图，悬停不会展开；收起期间系统工作区变化和后台高度更新不会触发自动展开，手动调整宽度由原生窗口立即记忆；启动、模式切换和第二次启动都会校验多显示器/DPI 下的可见 geometry，并唤醒既有窗口。
