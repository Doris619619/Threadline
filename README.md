...
- 首页 Daily 沿用待安排的分组列表、复选框与字号：父级标题右侧保留“记录”，子项缩进展示完整名称和预计/实际耗时；不再显示“今日结果”输入区，已有结果数据仍保留。父级可直接打卡，完成任一子项也算当天完成；取消父级会清除当天全部勾选，保留耗时与历史结果。子项预计分钟只读，实际逐项输入、父级汇总；无子项时直接填写 Daily 实际，旧父级额外耗时保留并明确计入合计。输入失焦保存，正式记录等待保存成功，失败保留输入并可重试。
