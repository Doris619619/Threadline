...
- **Windows 桌面三态工作流**：仅 Electron 检测到受限 Main Renderer bridge 时，完整工作台才显示迷你今日、工作站、最小化、原生最大化/还原和关闭。Web/PWA 始终保持完整工作台，历史桌面偏好也不会制造无作用入口。两种紧凑视图使用同一个始终置顶的主窗口并可互转；可拖到左右侧的 edge tab 只是紧凑视图的收起状态，单击恢复最近视图，悬停不会展开；启动、模式切换和第二次启动都会校验多显示器/DPI 下的可见 geometry，并唤醒既有窗口。
