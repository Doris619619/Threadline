...
“规划”默认进入雾蓝热力月历，点击某一天进入带周日期条的当天详情；手机和桌面采用相同浏览层级，返回时保留原月份与入口位置。日期为主、任务数量以较小文字显示，空日期不显示零值；深浅颜色表示安排数量，包含已完成任务。详情支持添加、编辑、完成、待安排转入及提前/推后改期，采用连续单列布局；预计只汇总未完成任务并标记未估时数量，浏览日期独立于首页。
