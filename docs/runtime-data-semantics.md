...
行内编辑同样固定进入编辑时的基准，失败不关闭输入；Enter 与失焦共用提交锁。字段冲突会停止这一轮后续排队命令，回读只用于恢复确认数据，不表示用户接受新的保存基准。最后草稿显示在当前账号会话中，用户复制、重新打开编辑后才形成新的意图。

回收站恢复保留原始 `deleted_at` 微秒令牌；其他用于显示的审计时间仍按原规则转换。工作站成功回包仅确认命令已执行，完整成员通过只读分页重新取得；读失败不发布部分集合，也不重发写命令。
