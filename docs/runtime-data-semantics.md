...
- iPhone 耗时与收尾表单使用至少 16px 字号和 44px 命中区域，长任务名换行。日程的项目标签与标题同行，时间、预计和实际分钟独立成第二排；收尾原生 dialog 显式居中并约束动态视口，只有内容区滚动，页脚按钮始终可见。选择“指定日期”才出现日期输入；明天、待安排和放弃沿用原 FormData 命令，取消与关闭按钮不提交。
