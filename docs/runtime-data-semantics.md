...
- 首页始终显示父子行、固定预计分钟和实际分钟，使用与待安排一致的复选框、字体与轻量分隔线，子项通过缩进表达层级。移除“今日结果”表单，已有 `result` 在保存与正式记录中原样保留，不变更数据库字段或清理历史。记录按钮位于父级标题右侧。预计分钟读取日期实例的清单快照；实际逐项填写，父级只汇总。没有子项的实例可直接填父级实际；旧实例已有父级分钟保留为明确标注的额外耗时，不批量清零或迁移到子项。
- iPhone 表单使用至少 16px 字号和 44px 命中区域，长任务名换行；窄屏页头允许日期工具栏换行，避免标题相撞。
- `DailyExecutionRow` 与 `useDailyExecution` 管理单个实例草稿；输入失焦保存，复选框直接保存，正式记录先等待该实例的最新提交完成。`saveDailyEntry` 返回 Promise，父子字段由 `save_daily_entry_bundle` 原子保存。草稿在失败和后台刷新时保留，执行区提供重试。已有正式历史仍保留记录时的快照，本次不回写历史。
