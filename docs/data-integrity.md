...
任务保存与实际账本读取分别确认。任务已提交后，耗时刷新失败只提示同步错误，不把已完成任务恢复为未完成。实际投入仍读取数据库账本，不使用乐观字段制造统计值。任务字段与单任务流转共享每任务队列，流转和收尾仍由数据库原子 RPC 完成。

行内编辑和弹窗都保留打开时的原值，提交差异并等待确认后关闭。字段冲突使当前队列失效，已排队的后续命令不会以重新读取的远端值自动重试。最后草稿保留在当前账号会话的提示中，供用户对照、复制和重新编辑；刷新或退出会清除会话草稿，提示明确告知这一点。普通网络错误继续读回确认状态，不自动重发写入。

任务 `deletedAt` 原样保留数据库时间字符串，恢复时作为精确并发令牌回传，不经过 JavaScript 毫秒格式化；数据库仍比较完整微秒。工作站写命令只执行一次，成功后通过分页读取完整成员，不能将受 PostgREST `max_rows` 限制的行集回包视为完整缓存。
