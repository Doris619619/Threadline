...
预计使用非负整数分钟，允许留空或清空为“待定”，显示为 `45min`、`1h`、`1h30min`。预计与同日的起止时间独立，修改时间不覆盖预计。任务新建、详情、行内预计遵守相同规则。待安排在任务名后显示预计，长标题允许换行；安排、移回待安排、移期、完成、恢复及收尾保留预计。历史实际账本继续使用原来的业务日和项目归属。

手机今日日程使用同一分组内连续双行，普通短标题约 88px；待安排约 52px，空组只保留标题与添加入口。手机输入至少 16px，操作保持 44px 点击区域；不限制页面缩放。桌面保留七列日程以及 Electron 共用前端。

待安排新增表单为预计字段指定独立整行，避免继承任务列表的 24px 勾选列。桌面首行为项目、事项内容与操作，下一行横排预计标签与输入；手机保留项目/操作、事项内容、预计三行和触控尺寸。新增态不占用完成框位置。布局回归覆盖 260/300/360px 桌面侧栏及 390px 手机，包含预计保存。
