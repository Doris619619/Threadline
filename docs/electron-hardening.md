...
`scripts/test-electron.mjs` 覆盖完整 Full→Workstation→Edge→Main、Edge 状态第二实例与确定退出。Windows PR CI 先通过 `pnpm test:electron` 验证开发壳的 Main/Renderer 行为，再构建 canonical `win-unpacked`。正式 fuses 会关闭 Node CLI inspect，所以 `pnpm test:electron:packaged` 不使用要求 Main inspector 的 Playwright Electron launcher，而通过 loopback Chromium CDP 验证最终 EXE 的 `threadline://app`、无 HTTP/file Renderer、CSP、受限 Preload 窗口切换、单实例与退出；runner 不启动 Next server，且在失败或超时后只结束本轮拥有的精确 Electron 进程树。
