...
| Phase 11      | Done | `refactor(desktop)：移除 Tauri 壳并完成 Electron 迁移交接`                                      | frozen install、hoisted linker、lint、typecheck、Vitest、Web/Electron build、49 个 E2E passed / 3 skipped、NSIS 产物均通过                                                | 交互式跨版本升级与任务栏固定仍需干净 Windows 用户环境复验                                                              |
...
**Phase 7 结果（2026-08-26）**：在任何 BrowserWindow 创建前设置 `AppUserModelID = com.doris619619.threadline` 与产品名 `Threadline`，与 electron-builder `appId` 保持一致。主进程获取 single-instance lock；第二实例聚焦当前 Edge 或 Main，异常时重建安全 Main。显示器事件由 Edge fallback 或当前 Main 恢复路径处理；Main renderer 崩溃/非预期关闭时，Edge 可见则后台重建 Main，否则 reload/reveal 安全 Main；用户显式关闭 Main 和 `window-all-closed` 均退出进程。`pnpm lint`、`pnpm typecheck`、`pnpm test`（13 passed）与 `pnpm desktop:compile` 均通过；deviations：None。
