...
- 总体状态：In Progress（Windows 最终验收复核中）。
...
| Phase 5       | Done        | `feat(desktop)：迁移三态窗口行为到 Electron Main`                                               | lint、typecheck、13 个 unit tests、Main/Preload compile 通过                                                                                                              | None                                                                                                                   |
| Phase 6       | Done        | `feat(desktop)：实现 Edge 轻量窗口与可见性故障恢复`                                             | lint、typecheck、13 个 unit tests、Main/Preload compile 通过                                                                                                              | None                                                                                                                   |
...
| Phase 9       | Done        | `test(desktop)：覆盖 Electron 启动切换与故障恢复`、`fix(windows)：收紧单实例启动与打包验收`     | typecheck、lint、Vitest（13 passed）、Electron window smoke（含共享 user-data 的第二实例退出断言）、desktop/mobile 浏览器 E2E（52 passed，3 个 desktop-only skipped）通过 | Electron smoke 的测试 user-data 改用系统临时目录，防止 Chromium 锁文件被 Next 构建扫描；生产沙箱配置不变               |
| Phase 10      | In Progress | `build(windows)：配置 Threadline NSIS 安装与发布产物`                                           | unpacked app、NSIS 安装/启动/卸载已通过；正在补验跨版本升级、任务栏固定与实际分组                                                                                         | 官方 artifact 下载仅在当前会话临时使用本机代理                                                                         |
| Phase 11      | In Progress | `refactor(desktop)：移除 Tauri 壳并完成 Electron 迁移交接`                                      | Tauri 清理与最终工程门禁已通过；等待 Phase 10 Windows 最终验收收口                                                                                                        | None                                                                                                                   |
...
**Phase 5 结果（2026-08-26）**：Full、Mini Today、Workstation 的所有 Electron 用户操作已通过 typed transition 交给 Main；Main 以同一个带 Windows 原生 frame 的 BrowserWindow 应用真实 bounds。Full 可最大化、非置顶且最小尺寸为 `800 × 560`；Mini Today 与 Workstation 可缩放、不可最大化、置顶，并分别限制为 `340–560 × 420–820` 与 `260–360 × 220–640` 的 logical pixel 区间。每个模式读取自身 persisted geometry，经显示器安全夹取后应用；浏览器/PWA 没有 bridge 时继续只更新业务 view。`pnpm lint`、`pnpm typecheck`、`pnpm test`（13 passed）与 `pnpm desktop:compile` 均通过；deviations：None。
...
**Phase 6 结果（2026-08-26）**：Edge 是独立的固定尺寸、无边框 BrowserWindow；显示 Edge 前先完成其加载和 reveal，再隐藏 Main。Edge 的构造/加载、`did-fail-load`、`render-process-gone`、`unresponsive`、非主动 `closed` 以及显示器增加、移除或指标变化均集中进入 `ensureVisibleSurface(reason)`：它先重建或显示 Main，再销毁故障 Edge；若 Main 重建失败则显示原生错误并退出。回退通过窄 IPC 通知 Renderer，将 persisted presentation 改为 `expanded`。`pnpm lint`、`pnpm typecheck`、`pnpm test`（13 passed）与 `pnpm desktop:compile` 均通过；deviations：None。
...
**Phase 7 结果（2026-08-26）**：在任何 BrowserWindow 创建前设置 `AppUserModelID = com.doris619619.threadline` 与产品名 `Threadline`，与 electron-builder `appId` 保持一致。主进程获取 single-instance lock；第二实例聚焦当前 Edge 或 Main，异常时重建安全 Main。复核发现第二实例虽调用 `app.quit()`，仍会继续注册 `whenReady` 初始化；已将启动与生命周期监听器限定在取得锁的分支中。最新 NSIS 安装器在开始菜单二次启动后稳定保留一个主进程，Renderer 命令行也携带统一 AppUserModelID。显示器事件由 Edge fallback 或当前 Main 恢复路径处理；Main renderer 崩溃/非预期关闭时，Edge 可见则后台重建 Main，否则 reload/reveal 安全 Main；用户显式关闭 Main 和 `window-all-closed` 均退出进程。`pnpm lint` 与 `pnpm desktop:compile` 通过；deviations：None。
...
**Phase 8 结果（2026-08-26）**：业务 Renderer 已不再导入 Tauri API 或探测 `__TAURI_INTERNALS__`；无 bridge 的浏览器/PWA 仅更新业务 view，并继续注册 Service Worker。Electron Main 角色跳过 Service Worker，Edge 在 Provider 前分流且不加载 PWA/业务运行时。`pnpm lint`、`pnpm typecheck`、`pnpm test`（13 passed）、`pnpm build`、`pnpm desktop:renderer` 与 `pnpm desktop:compile` 均通过；deviations：None。
...
**Phase 9 结果（2026-08-26）**：补齐了独占测试服务器、跨用例持久化 seed 隔离、Full→Mini→Workstation→Edge 的 Playwright Electron 冒烟验证、共享 user-data 的第二 Electron 实例退出断言、Edge reveal 8 秒超时回退，以及浏览器 desktop/mobile 新增、Daily、移期与持久化回归。`pnpm typecheck`、`pnpm lint`、`pnpm test`（13 passed）、`pnpm test:electron` 和浏览器 E2E（52 passed，3 个 desktop-only 用例在 mobile 项目跳过）通过。deviations：Electron 测试的 user-data 改存于系统临时目录并在结束时清理，以避免 Chromium 锁文件进入项目树并干扰 Turbopack；生产 `sandbox: true` 未改变。
...
**Phase 10 结果（2026-08-26）**：electron-builder 已配置统一的 `appId`、`productName`、正式 ICO、Windows x64 NSIS、用户级可选安装、开始菜单/桌面快捷方式、`release/` 输出目录和 `Threadline_${version}_${arch}-setup.${ext}` 命名。最新 `pnpm desktop:build` 生成 Windows x64 unpacked app 与 `Threadline_0.1.0_x64-setup.exe`（181,971,159 bytes）。在干净临时目录中，安装器静默安装返回 0、产出 `Threadline.exe` 与卸载器、应用可启动；开始菜单快捷方式指向该安装，二次启动稳定保留一个主进程，Renderer 命令行确认 `com.doris619619.threadline`。此前已完成 0.1.0 → 0.1.1 覆盖升级、启动和卸载验证；本次最新产物待验收结束后卸载并删除临时目录。

**Phase 10 deviations（2026-08-26）**：Electron 官方 artifact 直连网络超时时，仅在当前下载、postinstall 和打包子进程临时设置 `HTTP_PROXY` 与 `HTTPS_PROXY` 为 `http://127.0.0.1:7890`；先验证 Electron 官方 GitHub Release artifact 返回 HTTP 200，仍使用官方来源与默认 checksum 校验，并在每条流程结束后清除变量，未形成项目永久依赖。为隔离可复现的 pnpm collector SQLite 索引问题，工作区使用受 Git 忽略保护的本地 pnpm store，打包脚本只在 electron-builder 子进程期间创建并清理独立的临时 collector store；锁文件与运行时依赖未改变。跨版本覆盖升级已验证；手工任务栏固定和真实 Main/Edge 任务栏分组仍需在交互式干净 Windows 用户环境复验。
...
**Phase 11 结果（2026-08-26）**：已删除旧桌面壳、适配器、CLI 脚本、依赖和锁文件条目；`desktop:build` 现为 Electron NSIS 打包入口，Next 配置只保留 Web/PWA 服务模式与 Electron 静态 export。README、AGENTS 和桌面交互文档已切换为 Electron；除本迁移历史文档外，仓库不再有 Tauri、`TAURI_ENV_PLATFORM`、`__TAURI_INTERNALS__` 或 `.next-tauri` 引用。最终执行 `pnpm install --frozen-lockfile`、`pnpm config get nodeLinker`（`hoisted`）、Next 解析、`pnpm lint`、`pnpm typecheck`、`pnpm test`（13 passed）、`pnpm build`、`pnpm test:e2e`（49 passed，3 skipped）、`pnpm desktop:compile`、`pnpm desktop:renderer`、`pnpm test:electron`、`pnpm desktop:build:dir` 和 `pnpm desktop:build`；最终生成 Windows x64 unpacked app 与 `Threadline_0.1.0_x64-setup.exe`（181,844,422 bytes）。

**Phase 11 deviations（2026-08-26）**：本机 pnpm store SQLite 索引无法打开时，electron-builder 的 pnpm collector 无法工作；最终打包仅在进程期间使用干净临时索引并立即恢复生成 metadata，未改仓库配置或锁文件。官方 Electron artifact 直连超时时仍仅在当前下载/打包会话临时使用 `http://127.0.0.1:7890`，保持官方来源和默认 checksum，且每次结束均清除代理变量。干净临时目录的安装、启动和卸载已验证；真实跨版本升级、手工任务栏固定和多窗口任务栏分组仍未在交互式干净 Windows 用户环境复验。
